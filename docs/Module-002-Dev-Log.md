<!-- Markdown Docs: https://docs.github.com/en/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax -->
## Name: Richie Rojas
### Module: 002

<!-- Repeat the below as needed-->
### Date: [02/08/2026]

#### Goals for this Module
<!-- Example Template (include the brackets to make a checklist, fill them in as appropriate-->
- [x] Atari HW assignment
- [x] Choose game
- [x] PlayTest game
- [x] Finish the rest of the hw
- [x] Look over the slides and review them


#### Progress
- **What I accomplished**:
  - I looked over the slides and reviewed everything once again.
  - I chose Galaxian as my main game
  - I played the game and it was similiar to Galaga, (makes sense since Galaga is a sequel)
    
- **Challenges faced**:
  -  The initial websites the Professor gave were not working and so I had to find my own. Though to be honest I know Galaga so I'm familiar with the gameloop.
  -  The questions themselves were interesting yet they were sometimes difficult to come up with answers since some things didn't really apply like character development.
- **Solutions**:
  -  I found another website that allowed a bit of gameplay of the game as well as Galaga.
  -  I also watched some videos of the gameplay as well.

#### Learnings
-  I had to grow through many websites to gather information, though it honestly wasn't that difficult since it is a really popular game.
-  I learned that Galaxian was one of the first arcade games to integrate RGB colors into it. 
-  I also learned that Galaxian was extremely popular when the game initially came out and the sequel itself literally overshadowed the second game.
-  In the context of game development something that seemed simple in concept was extremely addicting for people to play and really highlights how a simple idea refined can change people's perceptions.

#### Free Thinking
- Brainstorm or reflect on design ideas, architecture patterns, or potential improvements.
- I found this assignment very enjoyable since I was able to explore retro games that I enjoyed a lot, playing both the prequel and sequel.


#### Next Steps
- Tasks or experiments to focus on during the next session.
- N/A
